...
export async function loadState(orgId) {
  ...
  const [org, clients, projects, employees, comp, licences,
         revenue, payroll, alloc, costs, periods, users] = await Promise.all([
    ...
    /* Sign-in accounts, not the payroll register - users and employees are
       separate things and nothing links them. Never selects password_hash. */
    sql`select u.email, u.full_name, u.is_active, m.role
          from c2c.users u
  ...
  return {
    ...
    users: users.map(u => ({
      email: u.email, name: u.full_name, role: u.role, active: u.is_active
    }))
  };
}

/* ---- the audit trail ------------------------------------------------------
   Read from v_audit, which resolves the uuids the triggers store into the
   codes and names the app speaks in. Paged with a keyset on id rather than an
   offset: the newest rows are what anyone reads, and an offset would drift as
   new ones arrive mid-page.                                                  */
export async function loadAudit(orgId, opts) {
  const sql = db();
  const lim = opts.limit;
  const before = opts.before;
  const month = opts.month ? opts.month + '-01' : null;

  const rows = await sql`
    select id, to_char(occurred_at, 'YYYY-MM-DD HH24:MI') as at,
           action, entity_table, to_char(period, 'YYYY-MM') as m,
           actor, actor_email, employee_code, employee_name,
           project_code, project_name, change
      from c2c.v_audit
     where org_id = ${orgId}
       and (${before}::bigint is null or id < ${before}::bigint)
       and (${month}::date is null or period = ${month}::date)
     order by id desc
     limit ${lim}`;

  return rows.map(r => ({
    id: String(r.id), at: r.at, action: r.action, table: r.entity_table, month: r.m,
    actor: r.actor, actorEmail: r.actor_email,
    employee: r.employee_name, employeeId: r.employee_code,
    project: r.project_name, projectId: r.project_code,
    change: r.change
  }));
}
...
export const OPS = {
  ...
  /* The salary register, amended from a month forward. Separate from
     setEmpCost, which restates one month's frozen payroll figure. */
  setEmployeeCtc:(sql, o, a) => sql`select c2c.op_set_employee_ctc(${o}, ${a.employeeId}, ${num(a.ctc)}, ${toDate(a.from)})`,
  ...
  /* A whole organization replaced from a backup file. One call, so it is one
     transaction: it reopens the closed months, rewrites everything, and closes
     them again - and if any of that fails the months close again with it.
     The engagement type is translated here rather than in SQL, because this
     file is the only place that knows the app's words for it. */
  restoreSnapshot: (sql, o, a) => {
    const snap = { ...(a.snapshot || {}) };
    snap.projects = (snap.projects || []).map(p => ({
      ...p, type: TYPE_TO_DB[p.type] || 'fixed_project'
    }));
    return sql`select c2c.op_restore_snapshot(${o}, ${JSON.stringify(snap)}::jsonb)`;
  },

  /* Passwords. Both take the caller from the session rather than the request
     body - who you are is not something the browser gets to assert. */
  setOwnPassword:   (sql, o, a, me) =>
    sql`select c2c.op_set_own_password(${o}, ${me.user_id}, ${me.token},
                                       ${a.current || ''}, ${a.next || ''})`,
  adminSetPassword: (sql, o, a, me) =>
    sql`select c2c.op_admin_set_password(${o}, ${me.user_id}, ${a.email || ''}, ${a.next || ''})`
};

/* Who may do what. Checked on the server; hiding a button is not a permission. */
export const WRITE_ROLES = ['owner', 'finance', 'manager'];
export const OWNER_ONLY  = ['closePeriod', 'setFiscalStart', 'deleteClient', 'deleteProject',
                            'deleteEmployee', 'restoreSnapshot', 'setEmployeeCtc'];
export const OWNER_ROLES = ['owner', 'finance'];
/* Stricter than OWNER_ONLY. Finance may touch every figure in the company but
   not identity: resetting an owner's password is how you become the owner. */
export const OWNER_STRICT = ['adminSetPassword'];
/* Allowed to any signed-in account, including a viewer. Changing your own
   password is not a write against the company's figures, and locking a
   read-only user out of their own credentials helps nobody. */
export const SELF_SERVICE = ['setOwnPassword'];
